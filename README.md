...
That is the whole thing. It installs what it needs, downloads the face models the first
time (about 40 MB), starts all three parts, and opens the officer screen in your browser.

The first run takes a few minutes because it downloads libraries. Later runs take about
thirty seconds.

When it is ready you will see:

```
Officer console      http://localhost:5173
Screening API        http://localhost:8080
Face verification    http://localhost:5000
```

Open **http://localhost:5173** in your browser.

**To stop everything:** run `stop.ps1`, or close the terminal windows it opened.

### If double-clicking does not work

Windows sometimes blocks scripts. Open PowerShell in this folder and run:
...
If you have Docker Desktop, this one command runs everything — no Java, Python or Node
needed:
...
First build takes 5–10 minutes. Then open **http://localhost:5173**.
...
```bash
# Backend
cd govtId-validation/backend
./mvnw spring-boot:run -Pembedded-mongo     # runs without needing MongoDB installed
./mvnw test                                 # 88 tests

# Frontend
cd govtId-validation/frontend
